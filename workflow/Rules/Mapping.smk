...
rule bwa_mem:
    input:
        R1 = f"{results_dir}/trimmed/{sample_name}_R1.fastq",
        R2 = f"{results_dir}/trimmed/{sample_name}_R2.fastq",
        idx=f"{data_dir}/{ref_genome}{ref_genome_ext}",
        flag=f"{results_dir}/flag/genome_indexed"
    output:
        f"{results_dir}/mapped/{sample_name}.bam"
    log:
        stdout = f"{results_dir}/logs/bwa_mem/{sample_name}.log",
        stderr = f"{results_dir}/logs/bwa_mem/{sample_name}_err.log"
    shell:
        """
        (bwa mem -t 2 {input.idx} {input.R1} {input.R2} | samtools view -bS - > {output}) > {log.stdout} 2> {log.stderr}
        """
